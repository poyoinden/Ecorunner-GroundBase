...
def workerB(latestvalues):
	while True:
		var1.set('Throttle: ' + str(latestvalues['throttle']))
		var2.set('RPM: ' + str(latestvalues['rpm']))
		speedKMH = rpmToKMHDisplay(latestvalues['rpm'])
		var3.set('KM/H: ' + str(speedKMH))
		var4.set('Current 1: ' + str(latestvalues['current']))
		var5.set('Current 2: ' + str(latestvalues['voltage']))
		root.update()
...
if __name__ == '__main__':
	...
	GPS = (0,0,0,0)
	#latestvalues = mgr.dict()
	latestvalues = dict()
	latestvalues['throttle'] = throttle
	latestvalues['rpm'] = rpm
	latestvalues['current'] = current
	latestvalues['voltage'] = voltage
	latestvalues['GPS'] = GPS

	jobs = [ multiprocessing.Process(target=workerA, args=(latestvalues,)) ]
	jobs.extend( [multiprocessing.Process(target=workerB, args=(latestvalues,)) ] )
